...
public class NumberComparisonGUIController implements Initializable {
    ...
    public void getEquation() {//make random equation
        ...
        char op = operator[rand.nextInt(3)];

        //save answer
        right_answer = ((op == '>') ? right > left : (op == '<') ? right < left : right == left);
    ...
    public void ansButton(ActionEvent event) throws IOException, InterruptedException {
        ...
        if (next.getText().equals("Answer")) {//answer mode
            ...
            try {
                boolean user_answer = Boolean.valueOf(ans.getText());

                //check answer
                if (right_answer == user_answer) {//correct
                    point.setText("POINTS: " + (++pts));//update points
                    msg.setText("CORRECT");
                    db.InsertGameRecord(firstNumber, secondNumber, opeator, 2, FXMLDocumentController.id); // Game number is 2
                } else {//wrong
